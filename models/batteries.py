...
def get_single_battery_by_id(db: Session, id: int=0):
    return db.query(Battery).filter(Battery.id == id).first()

def get_single_battery_by_code(db: Session, code: str = None):
    return db.query(Battery).filter(Battery.code == code).first()

def get_all_batteries(db: Session):
    return db.query(Battery).filter(Battery.deleted_at == None).order_by(desc(Battery.id)).all()

def get_all_batteries_by_mobility_device_id(db: Session, mobility_device_id: int = 0):
    return db.query(Battery).filter(and_(Battery.mobility_device_id == mobility_device_id, Battery.deleted_at == None)).all()

def search_batteries(db: Session, query: str = None):
    return db.query(Battery).filter(and_(or_(Battery.code.like('%' + str(query) + '%'), Battery.name.like('%' + str(query) + '%')), Battery.deleted_at == None)).order_by(desc(Battery.id)).all()
